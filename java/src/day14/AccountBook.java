...
public class AccountBook {
	...
	public void readItems() {
		...
		for(int i = 0; i<count; i++) {
			System.out.print(i+1+". "+items[i]); //여기에서 items[i]는 items[i].toSting()로 호출
		}
	}
	/* 기능 : 가계부에 새 내역을 추가하여 추가됐는지를 알려주는 메소드
	 * 		새내역(Item)이 주어지면 items에 추가하여 추가 됐는지를 알려주는 메소드
	 * 매개변수 : 새 내역 = > Item item
	 * 리턴타입 : 추가여부 => 성공/실패 = boolean
	 * 메소드명 : insertItem*/
	 ...
	 * 매개변수 : 삭제 할 번지 => int delIndex
	 * 리턴타입 : 삭제한 내역 => Item
	 * 메소드명 : deletItem*/
	
	public Item deletItem(int delIndex) {
		//잘못된 번지인 경우
