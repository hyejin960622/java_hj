...
public class Ex7_Exception_TryCatchFinally {
	...
	public static void main(String[] args) {
		...
		try {
			 ...
			 * - 예외가 발생하면 밑에 코드가 있던 없던 상관없이 예외를 처리 할 수 있는 
			 *   catch로 이동을 해서(예외를 처리할수 있는 catch가 없는 경우 처리 할 수 없어서 프로그램 중단) 예외를 처리함.
			 * - catch는 여러개가 올 수 있수있고(1개이상), Exception을 처리한다면 Exception은 무조건 마지막 catch에 배치해야함. 
			 * - catch를 작성할 때 부모 예외들을 자식아래로 배치*/
		...
		//에러 발생 위의 Exception e 때문에 ArithmeticException까지 올 일이 절대 없기 때문
