...
public class StudentManager implements ConsoleProgram{
	...
	public void excute(int menu) {
		switch(menu) {
		case 1:
			//학생 정보를 Scanner를 통해 입력받아 입력받은 정보로 이루어진 학생 객체를 받아오는 메소드
	...
	private Student inputStudent() {
		...
		return new Student(grade, classNum, num, name, kor, eng, math);
			...
			printMessage("입력에서 잘못된 값을 입력했습니다. 입력이 취소 됩니다.");
			//입력을 잘못했을때 입력 버퍼에 있는 내용들을 문자열로 가져옴(입력버퍼를 비우는 역할)
			scan.nextLine();
			return null;
		}
	...
	private void inserStudent(Student std) {
		if(std == null) {
			return;
		}
	...
	private Student inputSearchStudent() {
		try {
			System.out.println("검색할 학생정보를 입력하세요.");
	...
	//학년, 반, 번호가 있는 객체가 주어지면 해당 객체(modStd)에 이름, 국어, 영어, 수학을 
	//입력받아 수정하는 메소드
	
	private void inputAdditionStudnet(Student std) {
		if(std == null) {
			printMessage("학생정보가 없습니다.");
		}
		try {
		System.out.print("수정할 학생 정보를 입력하세요 : ");
		System.out.print("이름, 국어, 영어, 수학, 성적 입력 : ");
		String name = scan.next();
		int kor = scan.nextInt();
		int eng = scan.nextInt();
		int math = scan.nextInt();
