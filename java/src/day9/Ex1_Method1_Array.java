...
public class Ex1_Method1_Array {
	...
	public static void main(String[] args) {
		...
		for(  ; count < 3; ) {
			...
			// 배열에 count개만큼 랜덤한 수와 비교하여 있는지 없는지 확인해서 없으면(if) 
			int i =0;
			if(!contains(com, count, r)) {
				//배열 count번지에 랜덤한수 저장하고,
				com[count] = r;
				// count를 1증가
				count++;
			}
	...
	/* 기능 : 정수형 배열 2개가 주어지면 볼 갯수(같은 자리에서 일치하는 숫자의 개수)를 알려주는 메소드
	 * 매개변수 : 정수형 배열 2개 => int arr1 [], int arr2 []
	 * 리턴타입 : 볼 갯수 => 정수 => int
	 * 메소드명 : getBall*/
	public static int getBall(int arr1 [], int arr2[]) {
		...
		int ball = 0;
		//전체일치하는 갯수  =  볼 + 스트라이크의 갯수
		for(int tmp : arr1) {
			if(contains(arr2, arr2.length, tmp)) {
				ball++;
			}
		}
		//볼의 갯수 : 전체일차하는 갯수 - 스트라이크 갯수
